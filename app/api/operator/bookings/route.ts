 ...
 * Extras din GET ca să-l folosească și căutarea după număr (deep-link retur).
 */
async function withCoachData<
  T extends {
...
export async function GET(req: NextRequest) {
  ...
  // O SINGURĂ rezervare, după număr — pentru deep-link-ul „Bilet retur"
  // (/panou/retur/NR, deschis din butonul de pe davo.md/admin). Caută peste tot,
  // fără fereastra activ/arhivă: returul se face cel mai des DUPĂ cursa dus.
  const number = params.get("number")?.trim();
  if (number) {
    const booking = await prisma.booking.findUnique({ where: { bookingNumber: number }, select: SELECT });
    if (!booking) {
      return NextResponse.json({ success: false, error: "Rezervarea nu există" }, { status: 404 });
    }
    const [withCoach] = await withCoachData([booking]);
    return NextResponse.json({ success: true, booking: withCoach });
  }
  ...
  const withCoach = await withCoachData(bookings);
