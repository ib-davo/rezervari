...
// Iconițe de brand (path-uri simple-icons, viewBox 24×24) — lucide nu are
// iconițe de social, iar operatorii trimit biletul clienților mai ales pe
// WhatsApp/Viber, deci le desenăm inline, fără dependență nouă.
const SOCIALS: { name: string; bg: string; href: (url: string, text: string) => string; path: string }[] = [
  {
    name: "WhatsApp",
    bg: "#25D366",
...
/**
 * Acțiunile de după crearea unei rezervări: vezi/descarcă biletul + trimite-l
...
export function TicketShareActions({ ticketUrl, shareText }: { ticketUrl: string; shareText: string }) {
  const handleShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: shareText, text: shareText, url: ticketUrl });
      } catch {
        /* user a anulat share sheet-ul */
      }
    } else {
      try {
        await navigator.clipboard.writeText(`${shareText} ${ticketUrl}`);
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="flex items-center gap-3">
        <span className="text-xs font-semibold uppercase tracking-wider text-[color:var(--ink-500)]">
          Trimite pe:
        </span>
        {SOCIALS.map((s) => (
          <a
            key={s.name}
            href={s.href(ticketUrl, shareText)}
            target="_blank"
            rel="noopener"
            aria-label={`Trimite biletul pe ${s.name}`}
            title={s.name}
